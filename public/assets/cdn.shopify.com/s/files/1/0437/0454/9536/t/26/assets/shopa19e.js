(function($) {
  ...
  var buddha = {
    ...
    QuoteStyle2: function () {
      var QSlider2 = $('[data-quote-slider2]'),
          quotesNav2 = $('[data-quote-Nav2]'),
          blockSize = $('.quotes-section .quotes-slider').data('count');
   ...
   realtimeVisitor: function () {
    ...
    var el = $('.realTime');
      ...
      closeHeaderTop: function () {
        if ($.cookie('countdownCookie') == 'closed') {
          $('.header-top').hide();
        }
        $('.header-top a.close').bind('click',function(){    
          $('.header-top').css({"height": "0", "opacity": "0", "z-index":"-1"});
          $.cookie('countdownCookie', 'closed', {expires:1, path:'/'});
        });  
      ...
      $(".add-in-wishlist-js").each(function(){
        ...
        $(this).click(function(event){
          ...
          try
          {
            ...
            jQuery('.loading-wishbutton-'+id).show();
            jQuery('.default-wishbutton-'+id).remove();
            setTimeout(function(){ jQuery('.loading-wishbutton-'+id).remove(); jQuery('.added-wishbutton-'+id).show(); }, 2000);
            $(this).unbind();
          }
        ...
        if($.cookie(cookieName) != null && $.cookie(cookieName) != '__' && $.cookie(cookieName) != '') {
          ...
          for (var i=0; i<str.length; i++) {
            if (str[i] != '') {
              ...
              jQuery('.loading-wishbutton-'+str[i]).remove();
...
*/
 ...
 loadQuickViewSlider: function(product, r) {
            ...
            r.find(".quickview-featured-image").append('<a href="' + product.url + '"><img src="' + s + '" title="' + product.title + '"/><div style="height: 100%; width: 100%; top:0px; left:0; right:0; margin:0 auto; z-index: 2000; position: absolute; display: none; background: url(' + window.loading_url + ') 50% 50% no-repeat;"></div></a>');
                    ...
                    if(self.attr('disabled') !== "disabled") {
                        ...
                        Shopify.moveAlong = function() {
                            if (Shopify.queue.length) {
                                ...
                                var quantity = 1;
                                switch (window.ajax_method) {
                                    case "page":
                                        buddha.doAjaxAddToCart(data, handle);
                                        break;

                                    case "drawer":
                                        buddha.doAjaxAddToCart(data, title,image, quantity);
                                        break;
